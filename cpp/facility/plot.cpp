...
//bool recal   = false; /* Флаг пересчета положения пика */
...
VOID PlotRelax()
{
    EnterCriticalSection(&csSavedData);
        ...
        double delta    = 1000 / rate_DAQ;
        for(size_t i = 0; i < n; i++)
        {
            vData1.push_back(gate + i*delta); /* ms */
        }
        if(bspline::enable)
        {
            gsl_approx approx(vData1, SavedRelaxations[index_relax.load()], bspline::ncoeffs, bspline::order);
            for(size_t i = 0; i < n; ++i)
                vData2.push_back(approx(gate + i*delta));
        }
        else
        {
            for(size_t i = 0; i < n; ++i)
                vData2.push_back(SavedRelaxations[index_relax.load()][i]);
        }
...
VOID PlotDLTS(const gwin::gVector &xAxis, const gwin::gMulVector &yAxis)
{
    ...
    if(index_mode.load() == ITS)
        eps = 1e-9; /* Высокая точность определения для ITS */
    /* Поиск максимума методом золотого сечения */
    double a = xAxis.at(0); // Левый край
    double b = xAxis.at(xAxis.size()-1); // Правый край
    size_t n            = yAxis.size(); //Количество DLTS or ITS кривых
    for(size_t i = 0; i < n; i++)
    {
        const gwin::gVector &vData2 = yAxis[i];
        /* Определяем ориентацию пика */

        unique_ptr<gsl_approx> f_approx;
        unique_ptr<interp>     f_interp;
        auto f = [&](double arg)
        {
            return bspline::enable2 ? (*f_approx)(arg) : (*f_interp)(arg);
        };
        auto f_abs = [&](double arg){ return fabs(f(arg)); };

        if(bspline::enable2)
        {
            f_approx =  make_unique <gsl_approx> (xAxis, vData2, bspline::ncoeffs, bspline::order);
        }
        else
        {
            f_interp = make_unique <interp> (xAxis, vData2, gsl_interp_linear);
        }
        if(auto_peak_search.load() == true)
        {
            ...
            {
                double delta = 0.1 * (b-a);
                double from = (vPickData1[i] - delta > a ? vPickData1[i] - delta: a);
                double to = (vPickData1[i] + delta < b ? vPickData1[i] + delta: b);
                double tau = GoldSerch(from, to, eps, f_abs, MAXIMUM);
                vPickData1[i] = tau;
                vPickData2[i] = f(tau);
            }
        }
...
/*VOID dlts_OnRButtonDown(HWND hWnd, BOOL fDoubleClick, INT x, INT y, UINT keyFlags)
{
    if(vPickData1.empty() || vPickData2.empty())
        return;
    static CONST LONG MARK = 10; // Размер (радиус) области выделения пика в лог. ед
