...
class gsl_approx
{
    public:
        ~gsl_approx();
        gsl_approx(std::vector<double> const &vData1, std::vector<double> const &vData2,
                   size_t ncoeffs_  = 12,   /* number of fit coefficients */
                   size_t k         = 4);   /* breakpoint = ncoeffs + 2 - K*/
        double operator()(double x);

    private:
        const size_t N;         /* number of data points to fit */
        const size_t ncoeffs;   /* number of fit coefficients */
        const size_t nbreak;  /* nbreak = ncoeffs + 2 - k = ncoeffs - 2 since k = 4 */
