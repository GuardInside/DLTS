         ...
         case ID_BUTTON_APPLY_SETTINGS:
            {
                ...
                for (size_t i = 0; i < SendMessage(hWeightList, LB_GETCOUNT, 0, 0); i++)
                {
                ...
                write_settings();
                RefreshDLTS();
                InvalidateRect(hGraph_Arrhenius, NULL, FALSE);
            }
            break;
    }
}
//Оконная функция диалогового окна настроек корреляторов
/*BOOL CALLBACK csdlg_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
